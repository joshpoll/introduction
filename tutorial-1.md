...
In this case, our `Type` is `Set`, and we want all of our sets to be circles. We can make that happen by setting the `.icon` field to a shape object. Note that the use of `.icon` is arbitrary. You could also replace the field with whatever you prefer. For example, `.foo` would work the same. Penrose will still recognize that the field has been assigned to a shape object:
