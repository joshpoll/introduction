...
This is the specification for the shape **Circle**, and all the other shapes we have available are documented in the same way. You can see a sample diagram of what the shape looks like, and a table that lists out the different properties you can manipulate.
...
* Two terminals opened at the Penrose root directory
...
* The pattern for sending the three files we edited \(otherwise known as a **triple**\) to the Penrose server that we just launched in the previous step is as follows:`npx roger watch <path-to-sub> <path-to-sty> <path-to-dsl>`
  * So we need to replace each of the `<path-to>` with the actual path to the triple in the `tutorial1` directory. For example, if I had saved the `tutorials` repo at `~/repos/`, then I would run: `npx roger watch ~/repos/tutorial/code/tutorial1/twoSets.sub ~/repos/tutorial/code/tutorial1/twoSets.sty ~/repos/tutorial/code/tutorial1/setTheory.dsl`
* Refresh your Penrose window!

## Exercise

We have now covered the differences between and usage of the `.dsl`, `.sub` and `sty` files. We have provided 3 exercises for you to help solidify the basics. You can work on each of these within the existing files - no need to make new ones. **Hint:** Make use of the shape specs [here](https://github.com/penrose/penrose/wiki/Shape-library).
...
* Challenge 4: Keep 3 sets. For each set, represent `Set` as both a `Circle` and a `Square`. There should be 6 objects on your canvas \(Hint: you will need to initialize another [Shape](https://github.com/penrose/penrose/wiki/Shape-library) object!\).

![](.gitbook/assets/screenshot-from-2021-06-03-14-25-55.png)

After you are done, you can double-check your work with [solutions](https://github.com/penrose/tutorials/blob/main/solutions/tutorial1.md).
