...
Second, the term _numerical optimization_ is one kind of optimization, and a simple description is that we use some functions, that we call **energy / penalty functions**, to quantify how good our current solution is. The functions output a numerical value, hence _numerical_ optimization. Under the hood, **all constraint functions are implemented as energy functions**. 
